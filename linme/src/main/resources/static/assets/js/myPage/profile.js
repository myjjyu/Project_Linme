...
document.querySelectorAll("*[data-disabled]").forEach((button) => {
    button.addEventListener("click", (e) => {
        e.preventDefault();
        const targetSelector = button.dataset.disabled; // 버튼의 data-disabled 속성 값 (타겟 선택자)
        const targetElement = document.querySelector(targetSelector); // 타겟 요소 선택

        // 타겟 요소의 disabled 상태를 토글
        if (targetElement) {
            targetElement.disabled = !targetElement.disabled;
        }

        // 버튼의 UI 상태를 업데이트 (선택적으로 추가 가능)
        button.classList.toggle("active"); // 버튼에 활성화 상태를 나타내는 클래스 추가/제거
    });
});
